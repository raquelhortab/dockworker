...
function onTerminal(stream) {
  ...
  term.open(document.getElementById('terminal_wrapper'));
  stream.pipe(term);
...
function resize(term) {
  var x = document.getElementById('terminal_wrapper').parentElement.clientWidth / term.element.offsetWidth;
  var y = document.getElementById('terminal_wrapper').parentElement.clientWidth / term.element.offsetHeight;
  x = x * term.cols | 80;
  y = y * term.rows | 24;
  term.resize(x, y);
