...
function onConnect() {
  var addr = 'http://localhost:3030?type=filibuster';
  addr += "&args="+cmd;
  console.log("connecting to ", addr);
  var primus = new Primus(addr);
  console.log(primus);
  var terminal = primus.substream('terminal');
  var clientEvents = primus.substream('clientEvents');
  onTerminal(terminal);
  onClientEventsStream(clientEvents);

  primus.on('end', reconnect);
  primus.on('connect', hideLoader);
}
function disconnect() {
  console.log('DIS');
  message('term:dis');
